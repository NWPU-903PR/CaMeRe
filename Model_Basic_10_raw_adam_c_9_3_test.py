...
#drug feature embedding

class CaMeRe(nn.Module):
    def __init__(self,args,drop_d=0.15,drop_l=0.2):
        super(CaMeRe, self).__init__()
        self.drug_features = Drug_feature_embedding(dropout=drop_d)
        self.cell_features_sem = Cell_feature_embedding_sem()
        self.cell_features_var = Cell_feature_embedding_var()
        self.label_classifier = Label_classifier(drop_out=drop_l)
        ...
        #self.loss = PostCompensatedSoftmax(num_classes=2)
    ...
    def forward(self, mini_test, drug_features_cell, cell_features, codes_cell, number_train_task,
        ...
        # old_params_fs = parameters_to_vector(self.features_sem.parameters())
        ...
        #old_params_dc = parameters_to_vector(self.domain_classifier.parameters())
        ...
        for batch in range(batch_n):
            ...
            for batch_idx, data in enumerate(zip(data_train_patient_q, data1_train_patient_q,data_train_unpatient,data1_train_unpatient,data_train_patient, data1_train_patient)):
                ...
                #print(drug1_features_patient)
                patient_features_sem = self.cell_features_sem(x_query)
                #print(patient_features_sem)
                # feature_sem, kl_s,_,_ = self.features_sem(drug1_features_patient, drug2_features_patient,
                #                                       patient_features_sem)

                patient_samples_label, kl_s = self.label_classifier(drug1_features_patient, drug2_features_patient,
                                                      patient_features_sem)
                #print("kl_s:",kl_s)
                ...
                cell_samples_label, kl_s_c = self.label_classifier(drug1_features, drug2_features,cell_features_sem)
                ...
                loss_query_pa_id_s =(loss_query_pa_id_s_1 + loss_query_cell_id_s)
                # print("kl_s:", kl_s)
                # cell line
                # drug1_features, drug2_features = self.drug_features(x_query_q, x_query1_q)
                # cell_features_sem = self.cell_features_sem(x_query_q)
                ...
                # cell_features_var_mu_sig = cell_features_var_mu.reshape(cells_n, samples_n, -1)
                # cell_features_var_mu_sig = cell_features_var_mu_sig[:, 0, :]
                # cell_features_var_sigma_sig = cell_features_var_sigma.reshape(cells_n, samples_n, -1)
                # cell_features_var_sigma_sig = cell_features_var_sigma_sig[:, 0, :]
                ...
                # drug1_s_c = torch.cat([drug1_s] * features_sem_c_p.size()[0], dim=0)
                # drug2_s_c = torch.cat([drug2_s] * features_sem_c_p.size()[0], dim=0)
                ...
                # reversed_embedding_var = GradientReversalLayer.apply(embedding_var, 1)
                ...
                # patient_var_label = self.matrix_distance(features_var, device)
                ...
                # reversed_feature_mu = GradientReversalLayer.apply(features_sem_mu, 1)
                # reversed_feature_sigma = GradientReversalLayer.apply(features_sem_sigma, 1)

                # print("0:",torch.cuda.memory_allocated())
                task_label_sem = self.wasserstein_distance(task_feature_mu, task_feature_sigma, device)
                #print("task_label_sem:", task_label_sem)
                # print("1:",torch.cuda.memory_allocated())
                # loss_query_pa_indis_dom = torch.mean(domain_label_sem) + args.lamda_kl * kl_s
                loss_query_pa_indis_dom = domain_label_sem + task_label_sem + args.lamda_kl_s * (kl_se + kl_tc +kl_tup + kl_tp)

                #print("kl_se:", kl_se,"kl_tc:", kl_tc,"kl_tup:", kl_tup,"kl_tp:", kl_tp)
                ...
                backdoor_samples_label, kl = self.back_door_adjust(drug1_features_b, drug2_features_b,
                ...
                # drug1_features_b = torch.cat([drug1_features] * args.mean_backdoor, dim=0)
                # drug2_features_b = torch.cat([drug2_features] * args.mean_backdoor, dim=0)
                # cell_features_sem_b = torch.cat([cell_features_sem] * args.mean_backdoor, dim=0)
                ...
                loss_query_pa_invar_id = (loss_query_pa_invar_id_1 + ((cells_n_q*samples_n_q)/samples_n)*loss_query_cell_invar_id)
            ...
            # vector_to_parameters(old_params_fs, self.features_sem.parameters())
            ...
            #vector_to_parameters(old_params_ag, self.aggregation.parameters())
            vector_to_parameters(old_params_lc, self.label_classifier.parameters())
            #vector_to_parameters(old_params_dc, self.domain_classifier.parameters())
        ...
        G_train, P_train = total_labels_train.detach().numpy().flatten(), total_preds_train.detach().numpy().flatten()
        ...
        # loss_query_pa_id_s.backward(retain_graph=True)
        ...
        # for param in self.drug_features.parameters():
        #     print(param)
        ...
        if self.scheduler_v_a.get_last_lr()[0] > 1e-6:
            self.scheduler_v_a.step()
            #print("self.scheduler_d_a:",self.scheduler_d_a.get_last_lr())
    ...
    def inner_optimize(self, cells_n, samples_n_c, x_support, x_support1, x_support_unlabel, x_support1_unlabel,
        ...
        for k in range(update_step):
            ...
            # drug1_s_c = torch.cat([drug1_s] * features_sem_c_up.size()[0], dim=0)
            # drug2_s_c = torch.cat([drug2_s] * features_sem_c_up.size()[0], dim=0)
            ...
            # reversed_feature_mu_in = GradientReversalLayer.apply(features_sem_mu_in, 1)
            # reversed_feature_sigma_in = GradientReversalLayer.apply(features_sem_sigma_in, 1)
            ...
            backdoor_samples_label = backdoor_samples_label.view(-1, args.mean_backdoor, 2)
            backdoor_samples_label = torch.mean(backdoor_samples_label, dim=1)
            #backdoor_samples_label = self.label_classifier(backdoor_samples_feature)
            self.loss = Prior_ce_loss(2, prior_txt=label.cpu())
            ...
            # loss_classifer_in
            grads = torch.autograd.grad(loss_classifer_in, self.label_classifier.parameters(), retain_graph=True,allow_unused=True)
    ...
    def distance_matrix(self,z,p=2):
        ...
        #distance = torch.sum(0.5*(x - y) ** p + 0.5*(y - x) ** p, dim=3)
    ...
    def meta_test(self, X_cell,X_target,X_unpatient,test_loader_cell, test_loader1_cell, test_loader_unpatient, test_loader1_unpatient,
                                    test_loader_target, test_loader1_target, test_loader_cell_p, test_loader1_cell_p,cells_n, samples_n_c, save_list, device,args):
        ...
        for batch_idx,data_test in enumerate(zip(test_loader_cell_p, test_loader1_cell_p, test_loader_unpatient,test_loader1_unpatient)):
            x_support = data_test[0].to(device)
            x_support1 = data_test[1].to(device)
            x_support_unlabel = data_test[2].to(device)
            x_support1_unlabel = data_test[3].to(device)
            with torch.no_grad():
                # cell line
                drug1_features, drug2_features = self.drug_features(x_support, x_support1)
                cell_features_sem = self.cell_features_sem(x_support)
                # cell_features_var = self.cell_features_var(x_support)
                samples_label_cell, _, embedding_cell = self.label_classifier(drug1_features, drug2_features,
                                                                                  cell_features_sem)
                #print(torch.tensor(X_cell[:,0]).size())
                # print(np.shape(X_cell))
                X_cell_1= X_cell.reshape( cells_n*args.num_task*samples_n_c,-1)

                samples_label_cell = samples_label_cell.reshape(args.num_task, cells_n, samples_n_c, -1)
                samples_label_cell = samples_label_cell.permute(1, 0, 2, 3)
                samples_label_cell = samples_label_cell.reshape(cells_n* args.num_task * samples_n_c, -1)

                embedding_cell = embedding_cell.reshape(args.num_task, cells_n, samples_n_c, -1)
                embedding_cell = embedding_cell.permute(1, 0, 2, 3)
                embedding_cell = embedding_cell.reshape(cells_n* args.num_task * samples_n_c, -1)

                cell_feature_pre = torch.cat(( torch.tensor(X_cell_1[:,0]).to(device).view(-1,1), torch.tensor(X_cell_1[:,1]).to(device).view(-1,1),
                                               torch.tensor(X_cell_1[:,2]).to(device).view(-1,1), torch.tensor(X_cell_1[:,5]).to(device).view(-1,1),
                                               F.softmax(samples_label_cell,dim=1)), dim=1)
                cell_feature_pre = torch.cat((cell_feature_pre,embedding_cell), dim=1)
                save_list[0] = torch.cat((save_list[0],cell_feature_pre), dim=0)

                # cell line
                drug1_name = X_cell_1[:,1].reshape(-1,1)
                drug2_name = X_cell_1[:, 2].reshape(-1,1)
                # print(np.shape(drug1_name))
                X_cell_2 = X_cell_1.reshape(cells_n, args.num_task * samples_n_c, -1)
                cell_name = X_cell_2[:, 0, 0].reshape(-1,1)

                cell_features_sem_sig = cell_features_sem.reshape(args.num_task, cells_n, samples_n_c, -1)
                cell_features_sem_sig = cell_features_sem_sig.permute(1, 0, 2, 3)
                cell_features_sem_sig = cell_features_sem_sig.reshape(cells_n, args.num_task * samples_n_c, -1)
                cell_features_sem_sig = cell_features_sem_sig[:, 0, :]

                # unpatient
                unpatient_name = X_unpatient[:, 0].reshape(-1,1)
                drug1_unpatient_name = X_unpatient[:, 1].reshape(-1,1)
                drug2_unpatient_name = X_unpatient[:, 2].reshape(-1,1)
                # print(np.shape(unpatient_name))
                drug1_features_unpa, drug2_features_unpa = self.drug_features(x_support_unlabel, x_support1_unlabel)
                unpatient_features_sem = self.cell_features_sem(x_support_unlabel)

                features_sem_c_up = torch.cat((cell_features_sem_sig, unpatient_features_sem), dim=0)

                names_sem_c_up = np.concatenate((cell_name, unpatient_name), axis=0)
                # print(np.shape(names_sem_c_up))
                # drug pair
                drug1_name_s = np.concatenate((drug1_name, drug1_unpatient_name), axis=0)
                drug2_name_s = np.concatenate((drug2_name, drug2_unpatient_name), axis=0)
                ...
                #var
                cell_var = self.cell_features_var(x_support)
                cell_var_sig = cell_var.reshape(args.num_task, cells_n, samples_n_c, -1)
                cell_var_sig = cell_var_sig.permute(1, 0, 2, 3)
                cell_var_sig = cell_var_sig.reshape(cells_n, args.num_task * samples_n_c, -1)
                cell_var_sig = cell_var_sig[:, 0, :]
                ...
                backdoor_samples_label, _, backdoor_samples_embedding = self.back_door_adjust(drug1_features_b, drug2_features_b, cell_features_sem_b,
                                                                   cell_features_var_random)


                X_cell_1_b = X_cell_1.repeat(args.mean_backdoor, axis=0)

                backdoor_samples_label = backdoor_samples_label.reshape(args.mean_backdoor*args.num_task, cells_n, samples_n_c, -1)
                backdoor_samples_label = backdoor_samples_label.permute(1, 0, 2, 3)
                backdoor_samples_label = backdoor_samples_label.reshape(cells_n * args.mean_backdoor*args.num_task * samples_n_c, -1)

                backdoor_samples_embedding = backdoor_samples_embedding.reshape(args.mean_backdoor*args.num_task, cells_n, samples_n_c, -1)
                backdoor_samples_embedding = backdoor_samples_embedding.permute(1, 0, 2, 3)
                backdoor_samples_embedding = backdoor_samples_embedding.reshape(cells_n * args.mean_backdoor*args.num_task * samples_n_c, -1)

                cell_feature_pre = torch.cat((torch.tensor(X_cell_1_b[:, 0]).to(device).view(-1, 1),
                                              torch.tensor(X_cell_1_b[:, 1]).to(device).view(-1, 1),
                                              torch.tensor(X_cell_1_b[:, 2]).to(device).view(-1, 1),
                                              torch.tensor(X_cell_1_b[:, 5]).to(device).view(-1, 1),
                                              F.softmax(backdoor_samples_label, dim=1)), dim=1)
                cell_feature_pre = torch.cat((cell_feature_pre, backdoor_samples_embedding), dim=1)
                save_list[1] = torch.cat((save_list[1], cell_feature_pre), dim=0)

        #patient
        patient_features_sem_all = torch.Tensor().to(device)
        patient_features_var_all = torch.Tensor().to(device)
        for batch_idx, data_test in enumerate(zip( test_loader_target, test_loader1_target)):

            x_query = data_test[0].to(device)
            x_query1 = data_test[1].to(device)
            with torch.no_grad():
                # patient
                drug1_features_patient, drug2_features_patient = self.drug_features(x_query, x_query1)
                patient_features_sem = self.cell_features_sem(x_query)
                patient_features_var = self.cell_features_var(x_query)
                patient_samples_label, _, embedding_pa = self.label_classifier(drug1_features_patient, drug2_features_patient,
                                                                     patient_features_sem)
                patient_feature_pre = torch.cat((F.softmax(patient_samples_label,dim=1), embedding_pa), dim=1)

                save_list[2] = torch.cat((save_list[2], patient_feature_pre), dim=0)

                save_list[3] = torch.cat((save_list[3], x_query['y'].view(-1, 1).float().cpu()), 0)
                save_list[4] = torch.cat((save_list[4], F.softmax(patient_samples_label, dim=1)[:, 1].cpu()), 0)

                patient_features_sem_all = torch.cat((patient_features_sem_all, patient_features_sem), dim=0)
                patient_features_var_all = torch.cat((patient_features_var_all, patient_features_var), dim=0)

        save_list[2] = np.concatenate((X_target[:,0].reshape(-1,1), X_target[:,1].reshape(-1,1),
                       X_target[:,2].reshape(-1,1), X_target[:,4].reshape(-1,1),save_list[2].cpu().detach().numpy()), axis=1)

        # patient
        patient_name = X_target[:, 0].reshape(-1, 1)
        drug1_patient_name = X_target[:, 1].reshape(-1, 1)
        drug2_patient_name = X_target[:, 2].reshape(-1, 1)

        features_sem_c_up_p = torch.cat((features_sem_c_up, patient_features_sem_all), dim=0)
        #name
        # print(np.shape(drug1_name_s), np.shape(drug1_patient_name))
        names_sem_c_up_p = np.concatenate((names_sem_c_up, patient_name), axis=0)
        # drug pair
        drug1_name_s = np.concatenate((drug1_name_s, drug1_patient_name), axis=0)
        drug2_name_s = np.concatenate((drug2_name_s, drug2_patient_name), axis=0)
        # print(np.shape(drug1_name_s),np.shape(drug2_name_s))
        drug1_name_s_c = drug1_name_s.repeat(np.shape(names_sem_c_up_p)[0], axis=0)
        drug2_name_s_c = drug2_name_s.repeat(np.shape(names_sem_c_up_p)[0], axis=0)
        # print(np.shape(names_sem_c_up_p),np.shape(drug1_name_s)[0])
        name_sem_in = np.tile(names_sem_c_up_p.flatten(),np.shape(drug1_name_s)[0])
        # print(np.shape(drug1_name_s_c), np.shape(drug2_name_s_c),np.shape(name_sem_in))
        cell_name_sem = np.concatenate((drug1_name_s_c, drug2_name_s_c, name_sem_in.reshape(-1,1)), axis=1)

        drug1_s = torch.cat((drug1_s, drug1_features_patient), dim=0)
        drug2_s = torch.cat((drug2_s, drug2_features_patient), dim=0)
        drug1_s_c = drug1_s.repeat_interleave(features_sem_c_up_p.size()[0], dim=0)
        drug2_s_c = drug2_s.repeat_interleave(features_sem_c_up_p.size()[0], dim=0)
        features_sem_in = torch.cat([features_sem_c_up_p] * drug1_s.size()[0], dim=0)
        cell_sem = torch.cat((drug1_s_c, drug2_s_c, features_sem_in), 1)
        features_sem, _, _, _ = self.task_features_sem(cell_sem, cell_sem.size()[0])
        features_sem_save = np.concatenate((cell_name_sem, features_sem.cpu().detach().numpy()), axis=1)
        save_list[5] = features_sem_save
        #var
        cell_unpa_var = torch.cat((cell_unpa_var, patient_features_var_all), dim=0)
        features_var, _, _, _ = self.task_features_sem(cell_unpa_var, cell_unpa_var.size()[0])
        features_var_save = np.concatenate((names_sem_c_up_p, features_var.cpu().detach().numpy()), axis=1)
        save_list[6] = features_var_save
        ...
        for batch_idx, data_test in enumerate(zip(test_loader_target, test_loader1_target)):
            ...
            with torch.no_grad():
                ...
                for _ in range(patient_features_sem.size(0)):
                    index = torch.randperm(cell_unpa_var.size()[0])[:args.mean_backdoor]
                    features_var_random = torch.cat((features_var_random, cell_unpa_var[index]), dim=0)

                drug1_features_b = drug1_features_patient.repeat_interleave(args.mean_backdoor, dim=0)
                drug2_features_b = drug2_features_patient.repeat_interleave(args.mean_backdoor, dim=0)
                patient_features_sem_b = patient_features_sem.repeat_interleave(args.mean_backdoor, dim=0)

                backdoor_samples_label, _, backdoor_embedding_patient = self.back_door_adjust(drug1_features_b,
                                                                                              drug2_features_b,
                                                                                              patient_features_sem_b,
                                                                                              features_var_random)
                patient_feature_pre_back = torch.cat((F.softmax(backdoor_samples_label, dim=1), backdoor_embedding_patient), dim=1)

                save_list[7] = torch.cat((save_list[7], patient_feature_pre_back), dim=0)

        X_target_b = X_target.repeat(args.mean_backdoor, axis=0)
        save_list[7] = np.concatenate((X_target_b[:, 0].reshape(-1, 1), X_target_b[:, 1].reshape(-1, 1),
                                       X_target_b[:, 2].reshape(-1, 1), X_target_b[:, 4].reshape(-1, 1),
                                       save_list[7].cpu().detach().numpy()), axis=1)
