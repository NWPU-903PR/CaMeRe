...
fpath= 'data/feature/'
# drug pubchem cid
drug_feature=pd.read_table(os.path.join(fpath,"drug_id_cell.txt"),sep = "	",header=None)
#cell lines id / patient id/ pdx id
...
codes={'drugs': Mapping(drug_feature['DRUG_ID']),
       ...
       # 'PDXs':Mapping(cell_feature['CELL_ID'])}
       # 'Patients':Mapping(cell_feature['CELL_ID'])}



# drugs
DG= json.load(open(fpath + "drug_graph_cell.json"), encoding='utf-8')
...
pickle.dump(cell_feature, open(fpath+'cell_feature_900_comb.p', 'wb'))
pickle.dump(drug_feature, open(fpath+'drug_feature_cell_comb.p', 'wb'))
pickle.dump(codes, open(fpath+'codes_cell_comb.p', 'wb'))
# Patient
# pickle.dump(cell_feature, open(fpath+'patient_feature_900.p', 'wb'))
# pickle.dump(drug_feature, open(fpath+'drug_feature_patient.p', 'wb'))
# pickle.dump(codes, open(fpath+'codes_patient.p', 'wb'))

# PDX
# pickle.dump(cell_feature, open(fpath+'PDX_feature_900.p', 'wb'))
# pickle.dump(drug_feature, open(fpath+'drug_feature_PDX.p', 'wb'))
# pickle.dump(codes, open(fpath+'codes_PDX.p', 'wb'))